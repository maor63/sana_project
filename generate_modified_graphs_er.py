...
def write_leda_to_file(G, graph_name='syeast-05'):
    ...
    output_f = open(path + '/{0}.gw'.format(graph_name), 'wb')
...
n = 100
p = 0.05
total_posiple_edges = (n * (n - 1)) / 2.0
m = int(p * total_posiple_edges)
del_precent = 0.05
G2 = nx.gnm_random_graph(n, m=m, seed=seed)
G1_dense = nx.Graph(G2)



nodes = list(G1_dense.nodes)
G1_dense.remove_nodes_from(nodes[-1 * int(len(nodes) * del_precent):])
...
write_leda_to_file(G1_dense, 'er_n{}_p{}_del_{}nodes_dense'.format(n, p, del_precent))
write_leda_to_file(G1_equal, 'er_n{}_p{}_del_{}nodes_equal'.format(n, p, del_precent))
write_leda_to_file(G1_sparse, 'er_n{}_p{}_del_{}nodes_sparse'.format(n, p, del_precent))
write_leda_to_file(G2, 'er_n{}_p{}'.format(n, p))
...
write_leda_to_file(nx.complement(G2), 'er_n{}_p{}_complement'.format(n, p))
write_leda_to_file(nx.complement(G1_dense), 'er_n{}_p{}_del_{}nodes_dense_complement'.format(n, p, del_precent))